...
void ssd1331_draw_line(uint8_t chXpos0, uint8_t chYpos0, uint8_t chXpos1,
		uint8_t chYpos1, uint16_t hwColor) {
	...
	if (chXpos0 >= OLED_WIDTH || chYpos0 >= OLED_HEIGHT || chXpos1 >= OLED_WIDTH
			|| chYpos1 >= OLED_HEIGHT) {
		return;
	}
...
void ssd1331_draw_v_line(uint8_t chXpos, uint8_t chYpos, uint8_t chHeight,
		uint16_t hwColor) {
...
void ssd1331_draw_h_line(uint8_t chXpos, uint8_t chYpos, uint8_t chWidth,
		uint16_t hwColor) {
...
void ssd1331_draw_rect(uint8_t chXpos, uint8_t chYpos, uint8_t chWidth,
		uint8_t chHeight, uint16_t hwColor) {
	if (chXpos >= OLED_WIDTH || chYpos >= OLED_HEIGHT) {
		return;
	}
...
void ssd1331_fill_rect(uint8_t chXpos, uint8_t chYpos, uint8_t chWidth,
		uint8_t chHeight, uint16_t hwColor) {
	uint16_t i, j;
...
void ssd1331_draw_circle(uint8_t chXpos, uint8_t chYpos, uint8_t chRadius,
		uint16_t hwColor) {
 ...
 * @retval
 **/
void ssd1331_display_char(uint8_t chXpos, uint8_t chYpos, uint8_t chChr,
...
void ssd1331_display_num(uint8_t chXpos, uint8_t chYpos, uint32_t chNum,
		uint8_t chLen, uint8_t chSize, uint16_t hwColor) {
	uint8_t i;
	...
	for (i = 0; i < chLen; i++) {
		...
		if (chShow == 0 && i < (chLen - 1)) {
			if (chTemp == 0) {
				ssd1331_display_char(chXpos + (chSize / 2) * i, chYpos, ' ',
						chSize, hwColor);
				continue;
			} else {
				chShow = 1;
		...
		ssd1331_display_char(chXpos + (chSize / 2) * i, chYpos, chTemp + '0',
				chSize, hwColor);
	}
}
 ...
 * @retval  None
 **/
void ssd1331_display_string(uint8_t chXpos, uint8_t chYpos,
		const uint8_t *pchString, uint8_t chSize, uint16_t hwColor) {
	if (chXpos >= OLED_WIDTH || chYpos >= OLED_HEIGHT) {
		return;
	}

	for (int j = chXpos; j < OLED_WIDTH; j++) {
...
void ssd1331_draw_1616char(uint8_t chXpos, uint8_t chYpos, uint8_t chChar,
		uint16_t hwColor) {
	uint8_t i, j;
...
void ssd1331_draw_3216char(uint8_t chXpos, uint8_t chYpos, uint8_t chChar,
		uint16_t hwColor) {
	uint8_t i, j;
	uint8_t chTemp = 0, chYpos0 = chYpos;
...
void ssd1331_draw_bitmap(uint8_t chXpos, uint8_t chYpos, const uint8_t *pchBmp,
		uint8_t chWidth, uint8_t chHeight, uint16_t hwColor) {
